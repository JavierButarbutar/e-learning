...
class KonfirmasiKehadiranScreen extends StatefulWidget {
  ...
  const KonfirmasiKehadiranScreen({super.key, required this.qrCode});
...
class _KonfirmasiKehadiranScreenState extends State<KonfirmasiKehadiranScreen> {
  String? _latitude;
  String? _longitude;
  ...
  Future<void> _checkIn() async {
    ...
    if (provider.scanStatus == PresensiStatus.success &&
  ...
  void _showSuccessDialog(ScanResultModel result) {
    final isTerlambat = result.statusKehadiran == 'terlambat';

    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (_) => SuccessDialog(
        title: isTerlambat ? 'Presensi Terlambat' : 'Presensi Berhasil',
    ...
    return Scaffold(
      ...
      body: Consumer<PresensiProvider>(
        builder: (context, provider, _) {
          final loading = provider.isLoadingScan || _gpsLoading;
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                            ...
                            Expanded(
                              child: Text(
                                loading
                                    ...
                                    : _gpsError ?? 'Lokasi berhasil didapatkan',
                                style: TextStyle(
                                  fontSize: 12,
                                  color: _gpsError == null
                                      ? const Color(0xFF2E7D32)
                      ...
                      if (provider.scanStatus == PresensiStatus.error &&
                          provider.scanError != null) ...[
                        const SizedBox(height: 14),
                        _ErrorBanner(
                          message: provider.scanError!,
                          isSudahAbsen: provider.sudahAbsen,
                        ),
                      ],
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton.icon(
                          onPressed: (loading || _gpsError != null)
                              ? null
                              : _checkIn,
                          icon: loading
                              ? const SizedBox(
...
class _ErrorBanner extends StatelessWidget {
  ...
  const _ErrorBanner({required this.message, required this.isSudahAbsen});
