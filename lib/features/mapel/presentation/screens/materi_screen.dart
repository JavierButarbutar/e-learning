...
class _KuisSectionGroup extends StatelessWidget {
  ...
  const _KuisSectionGroup({required this.items, required this.namaMapel});
...
class _MateriTile extends StatelessWidget {
  ...
  Color get _accentColor {
    switch (item.type) {
      case MateriType.tugas:
        return const Color(0xFFF5A623);
      case MateriType.kuis:
        return item.tipeKuis.iconBgColor;
      default:
  ...
  void _onTap(BuildContext context) {
    if (item.type == MateriType.kuis) {
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) =>
              KuisDetailScreen(kuisId: int.parse(item.id)),
        ),
      );
    } else {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => DetailMateriScreen(item: item, namaMapel: namaMapel),
        ),
      );
    }
  }
